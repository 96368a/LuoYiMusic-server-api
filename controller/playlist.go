...
func SearchPlaylist(c *gin.Context) {
	...
	name := c.Query("name")
	playlists, count, err := services.SearchPlaylist(name, page.PageSize, page.Page, &model.User{})
	if err != nil {
...
func SelfPlaylist(c *gin.Context) {
	var page dto.Page
	err := c.ShouldBind(&page)
	...
	playlists, count, err := services.SearchPlaylist(name, page.PageSize, page.Page, &user)
	if err != nil {
		utils.Fail(c, 500, "内部错误", nil)
		return
	}
...
func DelPlaylist(c *gin.Context) {
	...
	sessionUser, _ := c.Get("user")
	user := sessionUser.(model.User)
	err := services.DelPlaylist(playlistDto.ID, &user)
	if err != nil {
		utils.Fail(c, http.StatusInternalServerError, err.Error(), nil)
		return
	}
